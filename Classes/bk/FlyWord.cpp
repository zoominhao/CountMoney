...
FlyWord* FlyWord::create(const char *filename, const float fontscale, Vec2 begin, float dis)
{
	FlyWord* ret = new FlyWord();
	//这样写更安全一些  
	if (ret && ret->init(filename, fontscale, begin, dis)){
		ret->autorelease();
		return ret;
	}
...
bool FlyWord::init(const char *filename, const float fontscale, Vec2 begin, float dis)
{
	if (!Sprite::init()){
		return false;
	}
